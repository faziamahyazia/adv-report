...
const formatInventoryQuantity = (row) => {
  ...
  const uom1 = String(product.uom_1 || product.unit || "kg").toLowerCase();
  const uom2 = String(product.uom_2 || (uom1 === "kg" ? "pcs" : "kg")).toLowerCase();

  const primaryValue = uom1 === "pcs"
    ? formatNumber(row?.base_quantity || 0)
    : formatKg(row?.quantity || 0);
