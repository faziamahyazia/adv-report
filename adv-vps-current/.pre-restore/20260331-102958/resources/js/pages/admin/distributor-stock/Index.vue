...
const openAdjustment = (row, type = "in") => {
  ...
  adjustmentForm.lot_number = "";
  adjustmentForm.expired_date = null;
...
const submitAdjustment = async () => {
  ...
  const remainingStock = Number(adjustmentForm.remaining_stock);
  ...
  const diffQty = Math.abs(remainingStock - currentStock);
  if (diffQty <= 0) {
  ...
  try {
    const response = await axios.post(route("admin.distributor-stock.adjust"), {
      ...
      type: remainingStock > currentStock ? "in" : "out",
      quantity: diffQty,
      remaining_stock: remainingStock,
      remaining_stock_unit: adjustmentForm.remaining_stock_unit || adjustmentRow.value.product?.uom_1 || adjustmentRow.value.product?.unit,
      ...
      >
        ...
        <template #item="props">
          <div class="col-12 q-pa-xs">
            <q-card flat bordered class="mobile-stock-card">
              <q-card-section class="q-pb-sm">
                <div class="row items-start justify-between q-col-gutter-sm">
                  <div class="col">
                    <div class="text-caption text-grey-7">Distributor</div>
                    <div class="text-weight-bold text-body1">{{ props.row.distributor?.name || '-' }}</div>
                  </div>
                  <div class="col-auto text-right">
                    <div class="text-caption text-grey-7">Stok Tersisa</div>
                    <div class="text-weight-bold text-primary text-body1">
                      {{ formatNumber(props.row.stock_quantity, 'id-ID', 2) }} {{ props.row.product?.unit || '-' }}
                    </div>
                  </div>
                </div>
              </q-card-section>
