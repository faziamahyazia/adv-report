...
const fyCompareOption = computed(() => {
  ...
  // Shorten month labels for radar (3 letters only)
  const shortLabels = labels.map(label => {
    const match = label.match(/^(\w{3})/);
    return match ? match[1] : label.substring(0, 3);
  });
  ...
  const radarConfig = {
    tooltip: { 
      ...
      formatter: params => {
        ...
        let html = `<div style="font-size:12px;font-weight:700;margin-bottom:6px">${seriesName}</div>`;
        labels.forEach((label, idx) => {
          const val = values[idx] || 0;
          html += `<div style="display:flex;justify-content:space-between;gap:12px;font-size:11px;margin:2px 0"><span>${label}:</span><b>Rp ${formatNumber(val, "id-ID", 0)}</b></div>`;
        });
        return html;
      }
    },
    ...
    radar: {
      indicator: shortLabels.map((label, idx) => ({ 
        name: label, 
        max: maxVal,
    ...
    series: [
      {
        ...
        data: [{
          ...
          itemStyle: { color: "#1976D2", borderColor: "#fff", borderWidth: 2 }
        }]
      },
      {
        name: previousLabel,
        ...
        data: [{
          ...
          areaStyle: { color: "rgba(144, 164, 174, 0.15)" },
        ...
        <q-card-section class="chart-shell-body">
          <div class="compare-kpi-grid q-mb-sm">
            ...
            <div class="compare-kpi-item compare-kpi-growth">
              <div class="compare-kpi-label">
                Growth YoY Total
                <q-icon name="info" size="14px" color="grey-6" class="q-ml-xs cursor-pointer">
                  <q-tooltip max-width="280px" class="text-body2">
                    YoY (Year-over-Year) Total adalah persentase pertumbuhan total penjualan tahun fiskal saat ini dibandingkan dengan tahun fiskal sebelumnya.
                  </q-tooltip>
                </q-icon>
              </div>
