...
const updateQuantity = () => {
  ...
  const baseQty = parseFloat(form.base_quantity) || 0;
  const weight = parseFloat(product.weight) || 0;
  const uom1 = (product.uom_1 || "").toLowerCase();
  const uom2 = (product.uom_2 || "").toLowerCase();

  if (baseQty <= 0 || weight <= 0) {
    form.quantity = 0;
    return;
  }
  ...
  if (uom1 === "pcs" && uom2 === "kg") {
    ...
    form.quantity = (baseQty * weight) / 1000;
  } else if (uom1 === "kg" && uom2 === "pcs") {
    // quantity(pcs) = base_quantity(kg) * 1000 / weight(gram)
    form.quantity = (baseQty * 1000) / weight;
  } else {
    // Default: assume base unit is pcs and secondary is kg
    form.quantity = (baseQty * weight) / 1000;
  }
};
...
const getSelectedProduct = () => {
  ...
  return page.props.products.find((p) => p.id === form.product_id);
};

const getBaseUOM = () => {
  const product = getSelectedProduct();
  return product ? (product.uom_1 || "pcs") : "pcs";
};

const getSecondaryUOM = () => {
  const product = getSelectedProduct();
  return product ? (product.uom_2 || "kg") : "kg";
};
</script>
          ...
          <q-card square flat bordered class="col">
            <q-card-section class="q-pt-md">
              ...
              <LocaleNumberInput
                ...
                :label="`Quantity (${getSecondaryUOM()})`"
                lazyRules
                readonly
                :maxDecimals="3"
                :disable="form.processing"
