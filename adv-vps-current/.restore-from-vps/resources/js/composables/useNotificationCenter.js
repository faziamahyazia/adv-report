...
export default function useNotificationCenter() {
  ...
  function maybeShowBrowserNotifications(notificationItems) {
    if (pushSupported) {
      // Jika web push aktif, browser notification di-handle oleh service worker.
      return;
    }
  ...
  async function openNotification(item, { navigate = true, showReadToast = true } = {}) {
    if (!item.read_at) {
      await markAsRead(item.id, { showToast: showReadToast });
    }
    ...
    window.Echo.private(realtimeChannelName).notification((notification) => {
      ...
      if (typeof window !== "undefined") {
        window.dispatchEvent(
          new CustomEvent("app:realtime-notification", {
            detail: notification || {},
          })
        );
      }
