...
function submit() {
  form.post(route("admin.notification.store"), {
    ...
    onError: () => {
      $q.notify({
        color: "negative",
        message: form.errors.target_role || form.errors.user_ids || form.errors.title || form.errors.message || "Pesan gagal dikirim.",
        position: "top",
      });
...
<template>
  <authenticated-layout>
    <template #title>Notifikasi Info</template>
      ...
      <q-card flat bordered class="notification-hero q-mb-md">
        <q-card-section>
          ...
          <div class="text-caption text-grey-7 q-mt-xs">
            Notifikasi akan masuk ke inbox aplikasi dan juga web push jika user sudah mengaktifkan izin browser.
          </div>
        </q-card-section>
      </q-card>
