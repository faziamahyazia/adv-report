...
class ActivityController extends Controller
{
    ...
    public function save(Request $request)
    {
        ...
        if (Auth::user()->role == User::Role_BS && (int) $item->user_id === (int) Auth::id()) {
            $bsUser = Auth::user();
            $agronomistUser = $bsUser->parent;

            if ($agronomistUser && $agronomistUser->role === User::Role_Agronomist) {
                app(FonteWhatsAppService::class)->sendActivityInputNotificationToAgronomist($agronomistUser, $bsUser, $item);
            }
        }
