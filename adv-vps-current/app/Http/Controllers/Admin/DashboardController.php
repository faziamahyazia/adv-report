...
class DashboardController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // Agronomist, ASM, and Admin all see the BS-activity overview dashboard
        if (in_array($request->user()->role, [User::Role_Agronomist, User::Role_ASM, User::Role_Admin])) {
    ...
    private function agronomistDashboard(Request $request, ?User $scopeUser = null)
    {
        ...
        $bsUsers = $user->role === User::Role_Admin
            ? User::where('role', User::Role_BS)->where('active', true)->orderBy('name')->get(['id', 'name'])
            : User::where('parent_id', $user->id)->where('role', User::Role_BS)->where('active', true)->orderBy('name')->get(['id', 'name']);
        ...
        if ($viewType === 'month') {
            ...
            $qNum      = $this->getQuarterFromMonth($month);
            $qMonths   = $fiscalQuarterMonths[$qNum];
            $mPosition = (int) array_search($month, $qMonths) + 1; // 1, 2, or 3

            $monthTargets = ActivityTarget::with('details')
                ->whereIn('user_id', $bsUserIds)
                ->where('year', $year)
                ->where('quarter', $qNum)
                ->get();

            $summary = $this->buildKpiSummary($bsUsers, $typeIds, $rows, $monthTargets, $activityTypes, $mPosition);
    ...
    private function buildKpiSummary($bsUsers, array $typeIds, array $rows, $targets, $activityTypes, ?int $monthPosition = null): array
    {
        // Build target map: user_id -> type_id -> target_qty
        // When $monthPosition is set (1/2/3), use that month's qty; otherwise sum all 3 months.
        $targetMap = [];
